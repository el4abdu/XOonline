...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const setupReadySystem = () => {
    ...
    const gameReadySystem = document.getElementById('gameReadySystem');
    ...
    // Hide ready system initially
    gameReadySystem.style.display = 'none';
    
    // Toggle player ready status
    const toggleReady = () => {
      readyState.isReady = !readyState.isReady;
      ...
      // Update Firebase
      if (gameRef) {
        gameRef.child('players').child(gameState.playerSymbol).child('ready').set(readyState.isReady)
          .catch(error => {
    ...
    const startCountdown = () => {
      readyState.countdown = 3;
      countdown.textContent = readyState.countdown;
      countdown.classList.remove('hidden');
      startGameBtn.disabled = true;
      readyBtn.disabled = true;
      
      // Start countdown interval
      readyState.countdownInterval = setInterval(() => {
        readyState.countdown--;
        countdown.textContent = readyState.countdown;
        
        if (readyState.countdown <= 0) {
          clearInterval(readyState.countdownInterval);
    ...
    const startGame = () => {
      ...
      gameRef.update({
        ...
        // Update UI
        updateStatusMessage(gameState.isPlayerTurn ? 'Your turn' : `${gameState.currentTurn}'s turn`);
      }).catch(error => {
    ...
    const listenForReadyStatus = () => {
      ...
      gameRef.child('players').on('value', snapshot => {
        ...
        // Update UI based on players count
        if (playerCount >= 2) {
          gameReadySystem.style.display = 'block';
          
          // Update player names and ready status
          if (playersInfo.length > 0) {
            player1Name.textContent = `${playersInfo[0].name} (${playersInfo[0].symbol})`;
            player1Ready.textContent = playersInfo[0].ready ? 'Ready' : 'Not Ready';
            player1Ready.className = playersInfo[0].ready ? 'ready' : 'not-ready';
          }
          
          if (playersInfo.length > 1) {
            player2Name.textContent = `${playersInfo[1].name} (${playersInfo[1].symbol})`;
            player2Ready.textContent = playersInfo[1].ready ? 'Ready' : 'Not Ready';
            player2Ready.className = playersInfo[1].ready ? 'ready' : 'not-ready';
          }
      ...
      // Listen for game started status
      gameRef.on('value', snapshot => {
        const data = snapshot.val();
    ...
    const isRoomCreator = () => {
      // Assume creator is the player who created the room
      // This logic depends on your game structure
      return gameState.isCreator === true;
    };
    ...
    // Initialize listen for player ready status
  ...
  const initComponents = () => {
    initUI();
    if (window.tacticalMode) {
      window.tacticalMode.init();
    }
    
    // Initialize player ready system
    const readySystem = setupReadySystem();
    
    // Store in global scope if needed
    window.readySystem = readySystem;
  };
